...
import { Layout, Modal, message, Card, BackTop } from 'antd'
...
@connect(({ loading, ReserveReportableInformation, resere }) => {
  return {
    listLoading: loading.effects['ReserveReportableInformation/fetch'],
    ReserveReportableInformation,
    resere,
  }
})
...
class IndexClassification extends Component {
  ...
  requestList = (params = {}) => {
    const { dispatch } = this.props
    dispatch({
      type: 'ReserveReportableInformation/fetch',
      payload: {
        pageNum: 1,
        pageSize: 10,
        ...params,
  ...
  handleAdd = values => {
    const { dispatch } = this.props
    dispatch({
      type: 'ReserveReportableInformation/add',
      payload: values,
      callback: () => {
  ...
  handleEdit = values => {
    const { dispatch } = this.props
    dispatch({
      type: 'ReserveReportableInformation/update',
      payload: values,
      callback: () => {
  ...
  handleEditInfo = async id => {
    ...
    const rowItem = await dispatch({
      type: 'ReserveReportableInformation/fetchDetail',
      payload: { id },
    })
    const reportInformation = rowItem ? rowItem.reportInformation : {}
    this.handleModalVisible(true, 'edit', reportInformation)
  }
  ...
  handleDelete = id => {
    ...
    Modal.confirm({
      ...
      onOk() {
        dispatch({
          type: 'ReserveReportableInformation/remove',
          payload: {
            id,
          },
          callback: () => {
  ...
  render() {
    ...
    const {
      listLoading,
      submitLoading,
      ReserveReportableInformation: { data },
      resere: { constructionType, statusList },
    } = this.props
    return (
      <div className="ReserveReportableInformation">
        <Layout>
