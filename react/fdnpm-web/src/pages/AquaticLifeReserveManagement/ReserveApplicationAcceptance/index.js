...
import { Layout, Modal, message, Card } from 'antd'
...
@connect(({ loading, ReserveApplicationAcceptance, resere }) => {
  return {
    listLoading: loading.effects['ReserveApplicationAcceptance/fetch'],
    ReserveApplicationAcceptance,
    resere,
  }
})
...
class IndexClassification extends Component {
  ...
  requestList = (params = {}) => {
    const { dispatch } = this.props
    dispatch({
      type: 'ReserveApplicationAcceptance/fetch',
      payload: {
        pageNum: 1,
        pageSize: 10,
        ...params,
  ...
  handleAdd = values => {
    const { dispatch } = this.props
    dispatch({
      type: 'ReserveApplicationAcceptance/add',
      payload: values,
      callback: () => {
  ...
  handleEdit = values => {
    const { dispatch } = this.props
    dispatch({
      type: 'ReserveApplicationAcceptance/update',
      payload: values,
      callback: () => {
  ...
  handleEditInfo = async id => {
    ...
    const rowItem = await dispatch({
      type: 'ReserveApplicationAcceptance/fetchDetail',
      payload: { id },
    })
    const checkApply = rowItem ? rowItem.checkApply : {}
    this.handleModalVisible(true, 'edit', checkApply)
  }
  ...
  handleDelete = id => {
    ...
    Modal.confirm({
      ...
      onOk() {
        dispatch({
          type: 'ReserveApplicationAcceptance/remove',
          payload: {
            id,
          },
          callback: () => {
  ...
  render() {
    ...
    const {
      listLoading,
      submitLoading,
      ReserveApplicationAcceptance: { data },
      resere: { constructionType, statusList },
    } = this.props

    return (
      <div className="ReserveApplicationAcceptance">
        <Layout className={globalStyles.globalStyles}>
        ...
        {modal.modalType === 'new' || modal.modalType === 'edit' ? (
          ...
          <FormView // 查看  审核
            title={modal.modalType === 'view' ? '详情' : '验收'}
            {...modal}
            loading={submitLoading}
            constructionType={constructionType}
            onCancelClick={() => this.handleModalVisible(false)}
